...
function CartItem({ number, title, thumb, item, price, id }) {
  ...
  let { cart, totalItems, setCart, handleAdd, handleRemove, setShowCart } =
    AppState();
  let button =
    "w-[30px] h-[30px] rounded-[50%] border-2 border-solid border-black font-black text-black";

  return (
    <div className="h-[100px] w-full container mx-auto bg-gray-100 flex mb-2 p-2 bg-[#eee] border-b-2  gap-2">
      <div
        className="h-full w-[35%]  flex justify-center items-center rounded-lg overflow-hidden"
        onClick={() => {
          navigate(`/product/${id}`);
          setShowCart(false);
        }}
      ...
      <div className="h-full w-[65%] flex justify-between ">
        <div className="flex flex-col justify-between">
          <h3
            className="w-[160px] truncate overflow-hidden font-bold hover:underline"
            onClick={() => {
              navigate(`/product/${id}`);
              setShowCart(false);
            }}
